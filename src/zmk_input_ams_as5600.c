 ...
 * BLE mode  – NOM in active mode; after 300 ms inactivity switch AS5600 to
 *             LPM2; after a further 1700 ms (2 s total) power-off the sensor
 *             (idle).  Any detected movement promotes back to active/NOM.
 *
...
/* AS5600 power-mode field (bits [1:0] of CONF byte 0). */
...
/* BLE: ms of inactivity before ACTIVE (NOM) → LOW_POWER (LPM2) */
...
enum zmk_input_ams_as5600_mode {
    ...
    ZMK_INPUT_AMS_AS5600_MODE_ACTIVE,     /* NOM mode, fast poll                  */
};
...
static int as5600_write_conf(const struct device *dev, uint8_t pm) {
    ...
    buf[0] = (buf[0] & (uint8_t)~AS5600_PM_MASK) | (pm & AS5600_PM_MASK);
...
static void work_handler_ble(struct zmk_input_ams_as5600_data *data) {
    ...
    case ZMK_INPUT_AMS_AS5600_MODE_IDLE:
        err = power_on_and_configure(dev, AS5600_PM_NOM);
        if (err) {
    ...
    case ZMK_INPUT_AMS_AS5600_MODE_LOW_POWER:
        ...
        if (!err && moved) {
            ...
            ble_enter_active(data);
            (void)as5600_write_conf(dev, AS5600_PM_NOM);
            (void)report_scroll(dev, pulses);
            schedule(data, BLE_ACTIVE_POLL_MS);
            return;
        }
